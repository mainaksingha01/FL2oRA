...
class TrainerBase:
    ...
    def switch_to_stage2(self):
        print(f"Switching to stage 2.")
        # set flag
        self.model.use_second_stage = True

        # Freeze stage1 params
        for param in self.stage1_params:
            param.requires_grad = False

        # set stage2 params (only ctx)
        self.stage2_params = [self.model.prompt_learner.text_features_zs]
        for param in self.stage2_params:
            param.requires_grad = True
        
        print(f"Stage 2 started")

        # Rebuild optimizer & scheduler for stage2
        self.optim = build_optimizer(self.stage2_params, self.cfg.OPTIM)
        self.sched = build_lr_scheduler(self.optim, self.cfg.OPTIM)
        # re-register in case TrainerBase relies on this mapping
        self.register_model("prompt_learner_stage2", self.model.prompt_learner, self.optim, self.sched)
...
class SimpleTrainer(TrainerBase):
    ...
    def model_inference(self, input):
        ...
        if isinstance(output, tuple):
            return output[0]  # use first tensor if multiple
        else:
            return output 
