...
class PlainMultiheadAttentionLoRA(nn.Module):
    ...
    def forward_module(
        ...
        """
        self.q_proj = self.q_proj.to(dtype=torch.float16, device="cuda")
        self.k_proj = self.k_proj.to(dtype=torch.float16, device="cuda")
        self.v_proj = self.v_proj.to(dtype=torch.float16, device="cuda")
        q = self.q_proj(query)
        k = self.k_proj(key)
        v = self.v_proj(value)
        ...
        self.proj = self.proj.to(dtype=torch.float16, device="cuda")
